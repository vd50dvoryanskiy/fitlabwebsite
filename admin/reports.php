...
function getClientsByWorkout($conn, $workout_type_id) {
    $query = "SELECT DISTINCT u.user_id AS client_id, u.full_name
              FROM Users u
              JOIN WorkoutAttendances wa ON u.user_id = wa.users_id
...
function getTrainerWorkoutCount($conn) {
    $query = "SELECT t.trainer_id, t.full_name, COUNT(s.schedule_id) AS total_schedules
              FROM Trainers t
              LEFT JOIN Schedules s ON t.trainer_id = s.trainer_id
...
function getActiveMembersWithoutWorkouts($conn) {
    $query = "SELECT u.user_id AS client_id, u.full_name, m.membership_type, m.start_date, m.end_date
              ...
              JOIN UserMemberships um ON u.user_id = um.user_id
              JOIN Memberships m ON um.membership_id = m.membership_id
              LEFT JOIN WorkoutAttendances wa ON u.user_id = wa.users_id
...
function getWorkoutsByRoomAndTime($conn, $room_id, $time) {
    $query = "SELECT s.*, wt.name as workout_name, t.full_name as trainer_name, r.name as room_name
              FROM Schedules s
              JOIN WorkoutTypes wt ON s.workout_type_id = wt.workout_type_id
              JOIN Trainers t ON s.trainer_id = t.trainer_id
              JOIN Rooms r ON s.room_id = r.room_id
              WHERE s.room_id = :room_id AND s.start_time = :time
            ...
            <div class="card">
                ...
                switch ($report_type):
                    ...
                    case 'clients_by_workout':
                        if ($workout_filter) {
                            ...
                            foreach ($workoutTypes as $wt) {
                                if ($wt['workout_type_id'] == $workout_filter) {
                                    $workoutName = $wt['name'];
                                    break;
                                }
                            }
                    ...
                    case 'members_expiring_soon':
                        ...
                        echo '<h3>Абонементы истекут в ближайшие 7 дней</h3>';
                        if (!empty($data)) {
                            echo '<div style="overflow-x:auto;"><table class="table"><thead><tr>
                                    <th>Имя</th>
                                    <th>Тип абонемента</th>
                    ...
                    case 'active_members_without_workouts':
                        ...
                        echo '<h3>Клиенты с активными абонементами, но без тренировок</h3>';
                        if (!empty($data)) {
                            echo '<div style="overflow-x:auto;"><table class="table"><thead><tr>
                                    <th>Имя</th>
                                    <th>Тип абонемента</th>
